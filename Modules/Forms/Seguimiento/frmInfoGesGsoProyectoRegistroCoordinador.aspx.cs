...
namespace InfogesEmape.Modules.Forms.Seguimiento
{
    public partial class frmInfoGesGsoProyectoRegistroCoordinador : System.Web.UI.Page
    {
		...
		protected void GridContratoValorizacionDet_HtmlRowPrepared(object sender, ASPxGridViewTableRowEventArgs e)
		{
            ...
            //string aprobado = (string)e.GetValue("APROBADO");
        ...
        protected void GridContratoValorizacionDet_HtmlDataCellPrepared(object sender, ASPxGridViewTableDataCellEventArgs e)
        {
            ...
            if (e.DataColumn.FieldName == "CANTIDAD")
            {
        ...
        protected void ASPxButton3_Click(object sender, EventArgs e)
        {
            ...
            using (MemoryStream ms = new MemoryStream())
            {
               ...
               linkGrid2Report.CreateDetailArea += new CreateAreaEventHandler(linkGrid2Report_CreateDetailArea);
                ...
                //phf.Header.Content.AddRange(new string[] { leftColumn, middleColumn, rightColumn });
                phf.Header.LineAlignment = BrickAlignment.Far;
                composLink.ExportToXls(ms);
                WriteResponse2(this.Response, ms.ToArray(), System.Net.Mime.DispositionTypeNames.Attachment.ToString());
            }

        }
        public static void WriteResponse2(HttpResponse response, byte[] filearray, string type)
        {
            response.ClearContent();
            response.Buffer = true;
            response.Cache.SetCacheability(HttpCacheability.Private);
            ...
            ContentDisposition contentDisposition = new ContentDisposition();
            contentDisposition.FileName = "SaldoDotacao.xls";
            contentDisposition.DispositionType = type;
            response.AddHeader("Content-Disposition", contentDisposition.ToString());
            response.BinaryWrite(filearray);
            HttpContext.Current.ApplicationInstance.CompleteRequest();
        ...
        protected void ASPxButton4_Click(object sender, EventArgs e)
        {
            ...
            //GridView dView = GridProyectoContratoCon.GetDetailView(e.RowHandle, 0) as GridView;

            exportAction = true;

            //dView.Columns["APROBADO"].Visible = false;

            //GridContratoValorizacionDet.DataColumns["APROBADO"].Visible = false;
            using (MemoryStream ms = new MemoryStream())
            {
                CompositeLink composLink = new CompositeLink(new PrintingSystem());
                //AGREGAR UN MARGEN AL INICIO
                ...
                linkGrid1Report.CreateDetailArea += new CreateAreaEventHandler(linkGrid1Report_CreateDetailArea);
                //composLink.CreateMarginalHeaderArea += new CreateAreaEventHandler(composLink_CreateMarginalHeaderArea);
                // PARA PONER SUPERVISOR Y PORCENTEJE GANADOR
                Link linkGrid2Report = new Link();
                linkGrid1Report.CreateDetailArea += new CreateAreaEventHandler(linkGrid2Report_CreateDetailArea);
                // PARA LA FECHA INICIO DE OBRA
                Link linkGrid3Report = new Link();
                ...
                // PARA CREAR UN ESPACIO PARA EL ENTRE LOS GRID
        ...
        void linkGrid2Report_CreateDetailArea(object sender, CreateAreaEventArgs e)
        {
            DataSet ds5 = new DataSet();
            ds5 = Code.Logic.Forms.Seguimiento.GsoProyectoRegistro.SearchByProyectoContrato((string)(Session["pIdProyecto"]), (string)(Session["IdDni"]));
            String contratista = ds5.Tables[0].Rows[0]["RAZON_SOCIAL_SUPERVISOR"].ToString();

            TextBrick tb = new TextBrick();
            tb.Text = "SUPERVISOR : " + contratista;
        ...
        void linkGrid3Report_CreateDetailArea(object sender, CreateAreaEventArgs e)
        {
            DataSet ds5 = new DataSet();
            ds5 = Code.Logic.Forms.Seguimiento.GsoProyectoRegistro.SearchByProyectoContrato((string)(Session["pIdProyecto"]), (string)(Session["IdDni"]));
            String contratista = ds5.Tables[0].Rows[0]["EMPRESA"].ToString();
            TextBrick tb = new TextBrick();
            tb.Text = "CONTRTISTA : " + contratista;
            tb.Font = new Font("Arial", 15);
            tb.Rect = new RectangleF(0, 20, 300, 70);
        ...
        protected void exporterValorizacion_RenderBrick(object sender, ASPxGridViewExportRenderingEventArgs e)
        {

            //if (e.Column.Grid.ID != "GridProyectoContratoCon")
            //{
                if (e.RowType == GridViewRowType.Header)
                {
                    e.BrickStyle.BackColor = Color.LightBlue;
                }
                else if (e.RowType == GridViewRowType.Data)
                {
                    e.BrickStyle.BackColor = Color.Azure;
                }

           // string metrado = e.GetValue("CANTIDAD").ToString();
          //  if (metrado == "0.00")
          //  {
          //      e.BrickStyle.BackColor = Color.FromArgb(144, 223, 203);
          //  }
            //if (e.GetValue("PAR_CANTIDAD").ToString() == "0.00" && (((e.Column as GridViewDataColumn).FieldName == "PAR_CANTIDAD") || ((e.Column as GridViewDataColumn).FieldName == "PAR_PRECIO") || ((e.Column as GridViewDataColumn).FieldName == "PAR_MEDIDA") || ((e.Column as GridViewDataColumn).FieldName == "TOTAL")))
                //e.Text = "";
            //}            
        }

        protected void GridContratoValorizacionDet_DataBound(object sender, EventArgs e)
        {
            if (exportAction)
            {                
                (sender as ASPxGridView).Columns["IDCONTRATOVALORIZACIONDET"].Visible = false;
                (sender as ASPxGridView).Columns["APROBADO"].Visible = false;
                (sender as ASPxGridView).Columns["SALDO_OLD_"].Visible = false;
            }
        }
    }	
}